[CmdletBinding()]
param()

$ErrorActionPreference = "Stop"
$root = Split-Path -Parent $PSScriptRoot
$bundle = Join-Path $root "db/flyway-ci-bundle"
$proposal = Join-Path $root "proposals/development-scoring-template/artifacts/scoring-template-seed.sql"
$suffix = [guid]::NewGuid().ToString("N").Substring(0, 12)
$container = "idea2strategy-scoring-seed-$suffix"
$database = "idea2strategy"
$user = "idea2strategy"
$password = "scoring-$suffix"

function Invoke-Docker([string[]]$Arguments) {
    $output = & docker @Arguments
    if ($LASTEXITCODE -ne 0) { throw "Docker command failed without accepting the scoring seed." }
    return $output
}

try {
    $null = Invoke-Docker @(
        "run", "-d", "--name", $container,
        "--health-cmd", "pg_isready -U $user -d $database",
        "--health-interval", "2s", "--health-timeout", "2s", "--health-retries", "30",
        "-e", "POSTGRES_DB=$database", "-e", "POSTGRES_USER=$user", "-e", "POSTGRES_PASSWORD=$password",
        "postgres:16-alpine"
    )
    $healthy = $false
    for ($attempt = 0; $attempt -lt 30; $attempt++) {
        $status = (& docker inspect --format "{{.State.Health.Status}}" $container 2>$null).Trim()
        if ($status -eq "healthy") { $healthy = $true; break }
        Start-Sleep -Seconds 2
    }
    if (-not $healthy) { throw "Temporary PostgreSQL did not become healthy." }

    $null = Invoke-Docker @(
        "run", "--rm", "--network", "container:$container",
        "-v", "${bundle}:/flyway/sql:ro", "redgate/flyway:11-alpine",
        "-url=jdbc:postgresql://localhost:5432/$database", "-user=$user", "-password=$password",
        "-connectRetries=30", "migrate"
    )
    foreach ($iteration in 1..2) {
        $null = Invoke-Docker @(
            "run", "--rm", "--network", "container:$container",
            "-e", "PGPASSWORD=$password", "-v", "${proposal}:/seed.sql:ro",
            "postgres:16-alpine", "psql", "-h", "localhost", "-U", $user, "-d", $database,
            "-X", "-q", "-v", "ON_ERROR_STOP=1", "--single-transaction", "-f", "/seed.sql"
        )
    }
    $count = (Invoke-Docker @(
        "exec", "-e", "PGPASSWORD=$password", $container, "psql", "-U", $user, "-d", $database,
        "-X", "-qAt", "-v", "ON_ERROR_STOP=1", "-c",
        "SELECT count(*) FROM competition.scoring_template_versions WHERE retired_at IS NULL AND version='development-2026-q3-v1';"
    ) | Select-Object -Last 1).Trim()
    if ($count -ne "4") { throw "Scoring seed did not create exactly four immutable Development templates." }

    $null = Invoke-Docker @(
        "exec", "-e", "PGPASSWORD=$password", $container, "psql", "-U", $user, "-d", $database,
        "-X", "-q", "-v", "ON_ERROR_STOP=1", "-c",
        "UPDATE competition.scoring_template_versions SET rules_document='{}'::jsonb WHERE id='cb2cd7ad-093d-4f1f-b76c-2762437ea101';"
    )
    $priorErrorPreference = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    & docker run --rm --network "container:$container" -e "PGPASSWORD=$password" -v "${proposal}:/seed.sql:ro" `
        postgres:16-alpine psql -h localhost -U $user -d $database -X -q -v ON_ERROR_STOP=1 --single-transaction -f /seed.sql 2>$null
    $collisionExitCode = $LASTEXITCODE
    $ErrorActionPreference = $priorErrorPreference
    if ($collisionExitCode -eq 0) { throw "A conflicting immutable scoring identity was not rejected." }

    [pscustomobject]@{ status = "passed"; templates = 4; replay = "idempotent"; collision = "rejected" } |
        ConvertTo-Json -Compress
}
finally {
    & docker rm -f $container 2>$null | Out-Null
}
