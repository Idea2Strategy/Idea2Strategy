...
if ($exportSource -match "status\s*=\s*'AVAILABLE'" -or
    ...
    $exportSource -notmatch '--version-id') {
...
$restorePosition = $importSource.IndexOf('Invoke-CheckedNative -FilePath "pg_restore"', [System.StringComparison]::Ordinal)
...
try {
    ...
    [System.IO.File]::WriteAllText((Join-Path $sandbox "storage-objects.csv"), "id,object_key`n11111111-1111-4111-8111-111111111111,historical/provider=fixture/part-00001.parquet`n")
    ...
    $storageHash = (Get-FileHash -LiteralPath (Join-Path $sandbox "storage-objects.csv") -Algorithm SHA256).Hash.ToLowerInvariant()
