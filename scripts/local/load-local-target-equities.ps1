...
try {
    ...
    $env:ALPACA_API_KEY = Read-EnvironmentValue 'ALPACA_API_KEY'
    $env:ALPACA_SECRET_KEY = Read-EnvironmentValue 'ALPACA_API_SECRET'
    Push-Location (Join-Path $repositoryRoot 'data-pipeline')
    try {
        & .\.venv\Scripts\python.exe -m apps.pipeline_worker.load_target_equity_history
        if ($LASTEXITCODE -ne 0) { throw "Canonical target-equity loader exited with $LASTEXITCODE" }
