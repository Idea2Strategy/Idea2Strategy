...
for required in archive archive_sha256 bundle_sha256 expected_migration_count database_host database_name database_port master_secret_arn policy_seed_sql policy_seed_sha256 scoring_seed_sql scoring_seed_sha256 region root_sha runtime_secret_arns_base64 work_directory artifact_bucket; do
...
# Export the same active US stock/ETF universe that backend strategy authoring
# reads. Keeping this in the private bootstrap transaction boundary prevents a
# hand-maintained two-symbol file from silently reaching the realtime gateway.
duplicate_symbol_count="$(psql -X -qAt -v ON_ERROR_STOP=1 -c "
  WITH eligible AS (
  ...
  SELECT count(*) FROM (SELECT symbol FROM eligible GROUP BY symbol HAVING count(DISTINCT id) <> 1) duplicates;")"
test "$duplicate_symbol_count" = '0'

instrument_mapping="$work_directory/instruments.json"
psql -X -qAt -v ON_ERROR_STOP=1 -c "
  ...
  | jq -S . >"$instrument_mapping"
instrument_count="$(jq 'length' "$instrument_mapping")"
test "$instrument_count" -ge 500
test "$instrument_count" -le 10000
jq -e 'to_entries | all(.key | test("^[A-Z0-9][A-Z0-9.-]{0,31}$")) and all(.value | test("^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$"))' \
  "$instrument_mapping" >/dev/null
instrument_mapping_sha256="$(sha256sum "$instrument_mapping" | cut -d' ' -f1)"
instrument_mapping_key='runtime/trading/instruments.json'
instrument_mapping_version="$(aws_retry s3api put-object --region "$region" --bucket "$artifact_bucket" \
  --key "$instrument_mapping_key" --body "$instrument_mapping" --query VersionId --output text)"
[[ -n "$instrument_mapping_version" && "$instrument_mapping_version" != None ]]

# The operator-authorized internal demonstration uses the paid Alpaca SIP
# entitlement. Evidence is short-lived and must be regenerated by a future
# bootstrap rather than being treated as a permanent legal assertion.
rights_evidence="$work_directory/alpaca-sip-rights.json"
rights_verified_at="$(date -u +'%Y-%m-%dT%H:%M:%SZ')"
rights_expires_at="$(date -u --date='+30 days' +'%Y-%m-%dT%H:%M:%SZ')"
jq -cn --arg provider alpaca --arg feed sip --arg verifiedAt "$rights_verified_at" \
  --arg expiresAt "$rights_expires_at" \
  '{provider:$provider,feed:$feed,verifiedAt:$verifiedAt,expiresAt:$expiresAt}' >"$rights_evidence"
rights_sha256="$(sha256sum "$rights_evidence" | cut -d' ' -f1)"
rights_key='runtime/trading/alpaca-sip-rights.json'
rights_version="$(aws_retry s3api put-object --region "$region" --bucket "$artifact_bucket" \
  --key "$rights_key" --body "$rights_evidence" --query VersionId --output text)"
[[ -n "$rights_version" && "$rights_version" != None ]]
...
jq -cn \
  ...
  --arg instrument_mapping_key "$instrument_mapping_key" \
  --arg instrument_mapping_version "$instrument_mapping_version" \
  --arg instrument_mapping_sha256 "$instrument_mapping_sha256" \
  --argjson instrument_count "$instrument_count" \
  --arg rights_key "$rights_key" \
