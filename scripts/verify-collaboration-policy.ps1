...
$authorityRelativePath = '.harness/product-authorities.yaml'
$authorityPath = Join-Path $repositoryRoot $authorityRelativePath
$ownerMetadataPath = Join-Path $localRoot 'policy/owner.yaml'
$authorityIds = @('user:kcrmin', 'user:pjy008008', 'user:Juwon-Na', 'user:hjcud')

foreach ($requiredFile in @($policyPath, $authorityPath)) {
  if (-not (Test-Path -LiteralPath $requiredFile -PathType Leaf)) {
    throw "Missing collaboration control file: $requiredFile"
  }
}

git -C $repositoryRoot check-ignore -q -- '.harness/local/project/policy/integrity.json'
if ($LASTEXITCODE -ne 0) { throw 'Operational content under .harness/local/ is not ignored by Git.' }
...
foreach ($trackedPath in @(git -C $repositoryRoot ls-files -- '.harness/local')) {
  if ($trackedPath -notin $allowedLocalFiles) { throw "Unexpected tracked local-harness content: $trackedPath" }
}

foreach ($legacyRoot in @('output', 'tmp', '.idea2strategy-local')) {
  $legacyPath = Join-Path $repositoryRoot $legacyRoot
  if ((Test-Path -LiteralPath $legacyPath -PathType Container) -and
      @(Get-ChildItem -LiteralPath $legacyPath -Recurse -Force -File).Count -gt 0) {
...
foreach ($link in $requiredLinks) {
  ...
  if (-not $content.Contains($link.Pattern)) { throw "Missing policy recovery link in $($link.Path)." }
}
...
foreach ($required in @(
  ...
  if (-not $authorityText.Contains($required)) { throw "Authority-registry requirement is missing: $required" }
}

foreach ($relativePath in @('AGENTS.md', '.agents/skills/use-project-harness/SKILL.md', 'docs/collaboration-policy.md')) {
  $content = Get-Content -Raw -Encoding utf8 (Join-Path $repositoryRoot $relativePath)
  foreach ($required in @('.harness/product-authorities.yaml') + $authorityIds + @('Git user.name and user.email never prove authority')) {
    if (-not $content.Contains($required)) { throw "Missing product-authority rule '$required' in $relativePath." }
  }
}

if (-not (Test-Path -LiteralPath $ownerMetadataPath -PathType Leaf)) { throw 'Ignored local product-owner metadata is missing.' }
...
foreach ($required in @('product_authorities: [user:kcrmin, user:pjy008008, user:Juwon-Na, user:hjcud]', 'contact_email_is_authority: false')) {
  if (-not $ownerMetadata.Contains($required)) { throw "Local product-owner metadata requirement is missing: $required" }
}
...
foreach ($file in $sharedFiles) {
  ...
  if ($content -match $emailPattern) { throw "Account email detected in shared file: $file" }
  foreach ($pattern in $credentialPatterns) {
    if ($content -match $pattern) { throw "Credential-like value detected in shared file: $file" }
  }
}

$actualHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $policyPath).Hash.ToLowerInvariant()
if (-not (Test-Path -LiteralPath $integrityPath -PathType Leaf)) { throw 'Local policy integrity metadata is missing.' }
$integrity = Get-Content -Raw -Encoding utf8 $integrityPath | ConvertFrom-Json
if ($integrity.sha256 -ne $actualHash) { throw 'Official collaboration policy differs from the authorized local integrity baseline.' }
...
if ($trackedInHead) {
  ...
  if ($LASTEXITCODE -ne 0 -and -not $AuthorizedPolicyChange) { throw 'Official collaboration policy has an unauthorized working-tree change.' }
...
[pscustomobject]@{
  ...
  authority_registry_verified = $true
  product_authorities = $authorityIds
  status = 'passed'
