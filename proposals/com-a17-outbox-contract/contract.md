# Outbox delivery contract v1 proposal

Status: isolated canonical proposal for Idea2Strategy/Idea2Strategy#129. This document and the accompanying DBML are not approved until product authority `user:kcrmin` reviews the exact commit.
...
This proposal selects current claim state on each outbox row plus append-only attempts, replay as a new row referencing an immutable original, and consumer receipt uniqueness by handler plus message ID while preserving producer key and payload hash evidence. These are product and service-contract choices recommended on issue #129. They must not be described as approved, integrated, or release-ready until `user:kcrmin` approves the exact Git commit through the configured GitHub review provider.
