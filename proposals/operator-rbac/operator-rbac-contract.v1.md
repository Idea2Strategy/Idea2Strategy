...
상태: [COM-A13 #130](https://github.com/Idea2Strategy/Idea2Strategy/issues/130)의 격리 제안. 이 문서와
`schema.draft.dbml`은 승인된 `contracts/**`와 `db/schema.dbml`을 대체하지 않는다. 정확한 commit과
보호 fingerprint에 대한 제품 권한자의 fresh 승인이 확인되고 DBML 변경 보류가 해제된 뒤에만 같은 내용을
정본으로 옮길 수 있다.
...
제품 권한자의 fresh exact-commit 승인이 이 제안의 유일한 의미 승인 gate다. 승인 전에는 canonical DBML,
`contracts/**`, Flyway migration 또는 backend 구현에 옮기지 않는다. 승인 시 actual permission catalog 항목을
추가할 필요는 없으며, 이 계약의 generic version/delegability/evidence 경계만 그대로 적용한다.
