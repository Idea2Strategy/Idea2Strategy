...
This isolated document proposes `official-room-scoring-decision.v1`. It does not become
approved product meaning until the exact commit and protected fingerprint receive
a fresh GitHub approval from `user:kcrmin` and that approved commit is merged.
...
The SHA-256 of this exact decision file and the repository protected fingerprint
are recorded in the approval PR. Any content change requires a new SHA-256 and a
fresh product-authority approval of the new exact commit.
