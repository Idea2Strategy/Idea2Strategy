...
1. Run `pwsh scripts/launch-status.ps1 -Owner <kcrmin|pjy008008|hjcud>`. It computes what is done from the repository itself and names the one next task. Do not pick a task by judgment when the script can name it; if the script says the owner is waiting, report who they are waiting on and stop instead of inventing work. (Claude sessions can start with `/start-work <owner>`; Codex sessions get the same entry as the `/start-work` custom prompt, installed into `$CODEX_HOME/prompts` by `scripts/initialize-local-harness.ps1` from the tracked `.agents/prompts/`. All three roads run this same script.)
