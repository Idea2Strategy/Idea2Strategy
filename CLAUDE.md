...
- Recover actual state with `scripts/verify-harness-consistency.ps1`, `git status --short --branch`, `git submodule status`, and `scripts/launch-status.ps1`. Treat chat history as a hint, not repository truth.
...
- Do not edit protected product, contract, policy, or authority-registry sources unless the repository authority gate permits it. Prepare an isolated proposal when approval is unavailable.
