...
export function NotificationsView() {
  return <div className="page narrow-page"><PageHeading eyebrow="INBOX" title="알림" description="봇 운영과 방 일정에 영향을 주는 사건을 한곳에서 확인합니다." actions={<Button icon={Check}>모두 읽음</Button>} /><Panel className="notification-panel" title="최근 알림" subtitle="미확인 2개">{notifications.map((item) => <button className={`notification-row ${item.unread ? 'unread' : ''}`} key={item.title}><span className="notification-mark"><BellRing size={17} /></span><span><small>{item.kind}</small><strong>{item.title}</strong><p>{item.detail}</p></span><time>{item.time}</time></button>)}</Panel></div>;
}

export function AccountView() {
  return <div className="page narrow-page"><PageHeading eyebrow="ACCOUNT & ACCESS" title="계정과 보안" description="로그인 수단과 서버 실행 확인 정책을 관리합니다." />
...
export function AdminView() {
  return <div className="page"><PageHeading eyebrow="RBAC · OPERATOR" title="운영 관리" description="권한이 있는 운영자만 데이터 품질과 시장 이벤트 반영 작업을 처리합니다." actions={<Status tone="neutral">OPERATOR</Status>} />
