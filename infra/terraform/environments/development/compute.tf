resource "aws_instance" "service" {
  ...
  user_data_base64 = base64gzip(templatefile("${path.module}/templates/ec2-user-data.sh.tftpl", {
    ...
    operator_auth_mfa_claim_name                = local.operator_auth_mfa_claim_name
    operator_auth_allowed_mfa_claim_values      = join(",", sort(tolist(local.operator_auth_allowed_mfa_claim_values)))
