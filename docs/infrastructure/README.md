...
현재 목표 구조의 기준은 Core·Trading·Compute EC2 세 대이며 공개 진입점은 Route 53 → ALB → Core EC2로 확정한다. Queue와 Redis는 분리하되 Queue 제품과 Redis 운영 제품은 아직 결정하지 않았으며, 기존 2 EC2·로컬 Redis 문서는 결정 이력으로만 본다.
