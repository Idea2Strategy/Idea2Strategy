...
**미결(F-4, 높음 — 출시 차단)**: 정본 체크리스트 §0이 명시한 "운영 전 실제 서비스 사용 범위에 대한 **Alpaca의 서면 확인**과 권리 gate"는 외부 법적 행위로, 저장소 안에서 닫을 수 없다. `policy.legal.block-uncertain`에 따라 이 확인 없이는 실데이터 기능을 공개하지 않는다. INT12(v1.0 릴리스) 전 완료 필수.

## 6. 미결 항목 요약

| # | 심각도 | 항목 | 귀속 |
|---|---|---|---|
| F-4 | 높음 | Alpaca 사용 범위 서면 확인 (외부·법적) | 제품 권한자, INT12 전 |
...
다섯 축 모두 검토를 수행했고 표본 수준의 위반은 발견하지 못했다. 그럼에도 INT08 카드는 **닫지 않는다**:

1. F-4(Alpaca 서면 확인)는 외부 행위이며 `policy.legal.block-uncertain`이 요구하는 확인 그 자체다.
2. INT 구간의 완료 의미(§7 "실제 서비스 조합에서 통과")는 배포된 조합에서의 재검증을 포함하며, F-2·F-3은 그 재검증(A90·INT10)에 귀속된다.

닫는 조건: F-4의 서면 확인이 기록되고, A90·INT10 검증이 F-1~F-3을 흡수하면 이 문서를 갱신하고 카드를 닫는다.
