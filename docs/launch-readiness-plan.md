...
**할 일:** 위 두 줄을 `docs/development-start-guide.md`에 못박는다(현재 문서에 이 순서가 명시돼 있는지 확인 후 없으면 추가). 코드 변경은 필요 없다.
...
| INT05 운영자 E2E | P1 | §2.2 | 운영자 OIDC·SES는 로컬에서 안 되므로 **AWS에서** |
| INT06 장애·재기동·중복 전달 | P2 | §2.1 §3.1 | **§3.1이 여기 걸린다** — 재기동 위상 문제를 안 고치면 이 카드가 정직하게 통과할 수 없다 |
| INT07 성능·용량 | P2 | **차단됨** | §6 참조 — 목표치가 없어 합격 기준이 없다 |
| INT08 보안·개인정보·법적 표현 | P1 | — | 1차 검토 완료(`docs/reviews/int08-...md`). 잔여 3건: 인증 전수(A90), 성과 카피 전수(INT10), admin-mcp read 전수(A90) |
...
```
P1: 2.3(flyway 번들) → 2.1(TRADING_* 배선) → 2.2(방·운영자 큐/플래그) → 2.6(AWS relay)
                                    ↓
P2:                        2.4(30m 포함 시장 데이터) → 2.5(feature 백필) → 3.1(재기동 위상)
                                    ↓
P3:                                        3.3(UI 정리) → INT03 · INT10
                                    ↓
P1: INT02 → INT01 → INT04 → INT05 → INT08 → INT09 → A90 → A91 → INT11 → INT12
동시: §6(SLO 결정) — INT07의 선행이므로 늦어질수록 릴리스가 밀린다
```

2.1과 2.4는 **서로 독립**이라 P1·P2가 병행할 수 있다. 2.5는 2.4에 의존한다. INT03은 2.1·2.4·2.5 전부에 의존한다.
