...
Idea2Strategy must fail closed when an actor attempts to change canonical product meaning without product-owner authority. The product authorities are the GitHub accounts `user:kcrmin` and `user:pjy008008`. The known owner email remains ignored local contact metadata, not authentication evidence.
...
- Normalized product authorities: `user:kcrmin` and `user:pjy008008`, with equal scope.
...
4. the verified approving subject is a configured product authority (`user:kcrmin` or `user:pjy008008`);
...
1. Enable native Stackcord governance with `user:kcrmin` and `user:pjy008008` as the configured authorities.
