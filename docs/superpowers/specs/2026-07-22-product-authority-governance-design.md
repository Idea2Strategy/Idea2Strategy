...
- Sole normalized product authority: `user:kcrmin`.
- Protected semantic kinds: `product`, `policy`, `business`, and `contract`.
...
4. the verified approving subject is `user:kcrmin`;
...
1. Enable native Stackcord governance with `user:kcrmin` as the only authority.
...
- Actor other than `user:kcrmin`: allow only an isolated proposal.
