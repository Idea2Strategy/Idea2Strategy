...
- 개발 단계에서는 SES를 사용하지 않으므로 이메일·비밀번호 가입은 즉시 `ACTIVE`로 진입한다. `PENDING_VERIFICATION`과 이메일 인증 자료구조는 추후 메일 발송 재도입을 위해 유지하지만, 현재 가입·로그인의 선행조건으로 사용하지 않는다. 이 결정은 `user:kcrmin`의 2026-08-14 지시에 따른다.
