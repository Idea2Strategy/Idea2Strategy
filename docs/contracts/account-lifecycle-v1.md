...
- 탈퇴 요청은 최근 단계 상승 인증을 요구한다. 성공 요청 시 `withdrawal_requested_at`을 서버 수신 시각으로, `cancellation_deadline_at`을 정확히 `withdrawal_requested_at + 30일(30 × 24시간)`로 저장한다.
...
휴면 기준은 **최근 성공 인증 시각부터 연속 12개월**이다. 이 12개월 기준은 법적 의무에 대한 주장이 아니라 제품 운영 제안이며, 제품 권한자가 병합 전에 확인해야 한다.
...
- 숫자 보존기간은 제품·법무가 승인한 정책 레지스트리에서만 온다. 승인된 기간이 없는 분류는 `CLOSED` 전환은 허용하되 물리 삭제를 보류하고 `RETENTION_POLICY_MISSING` 운영 경보를 낸다.
...
다음 항목은 구현 전에 제품 권한자 또는 해당 책임자의 확인이 필요하다.

1. 휴면 기준을 최근 성공 인증 후 12개월로 채택할지, 그리고 UTC 달력 연산을 사용할지.
2. 이메일과 OIDC 식별자의 재사용 격리 기간을 `CLOSED` 후 30일로 채택할지.
3. 데이터 분류별 실제 숫자 보존기간, 처리 방식, 근거와 승인자를 담은 retention policy v1.
4. 어떤 legal hold 유형이 `blocks_identifier_reuse`를 설정할 수 있는지와 해제 권한자.
5. trading의 포지션·잔액, competition의 보상·순위, 공개 전략 소유권을 종료할 구체적 도메인 절차.
6. 단계 상승 인증의 허용 수단과 최근 인증 인정 시간.

위 결정이 남아 있어도 구현자는 상태 전이, 즉시 접근 폐기, 사건 불변식, 기본 거부, 정책 버전 고정 규칙을 약화해서는 안 된다. 숫자 보존기간이나 금융 정리 행위를 임의로 코드에 넣지 않는다.
