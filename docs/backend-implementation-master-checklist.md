...
2. 실제 Git·서브모듈·작업 원장을 읽고 진행 중 작업과 의미 충돌을 확인한다.
3. 해당 카드를 루트 상위 Issue로 두고, 담당 저장소의 하위 Issue 하나를 실행 가능한 작업으로 정의해 작업 브랜치를 예약한다.
...
- [x] **COM10 — 병렬 작업 소유권 확인**: A~F의 경로·스키마·계약 producer를 확정하고 작업 원장과 Git 상태로 의미·migration·workspace·root pointer 충돌과 병합 순서를 확인한다.
